...
export async function GET(req: Request) {
  ...
  const base: any = { ok: true, version: '0.3.0',
    build: { sha: (process.env.VERCEL_GIT_COMMIT_SHA || 'local').slice(0, 7), message: process.env.VERCEL_GIT_COMMIT_MESSAGE || null, deployment: process.env.VERCEL_DEPLOYMENT_ID || null, env: process.env.VERCEL_ENV || 'local', url: process.env.VERCEL_URL || null, built_at: process.env.BUILD_TIME || null }, providers: providerStatus(), persistent: persistent(), serverKeyStorage: canStoreServerKeys(), serverKeys: await serverKeyNames(), connected: reg.filter(c => c.status === 'CONNECTED').map(c => c.id), missing: reg.filter(c => c.status !== 'CONNECTED').map(c => c.id) };
