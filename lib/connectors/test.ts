...
export async function testConnector(id: string): Promise<{ ok: boolean; message: string }> {
  ...
  try {
    if (c.kind === 'model' || c.id === 'web_search') { const p = getProvider(c.provider === 'anthropic' ? 'anthropic' : 'openai'); const r = await p.call({ system: 'Reply with the single word OK.', content: [{ type: 'text', text: 'ping' }], maxTokens: 5 }); return { ok: /ok/i.test(r.text), message: r.provider + ' · ' + r.model + ' · ' + r.ms + 'ms' }; }
